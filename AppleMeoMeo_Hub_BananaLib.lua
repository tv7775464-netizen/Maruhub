--[[ AppleMeoMeo Hub
     Target UI: BananaLib
     Source: jssj.lua
     Title: AppleMeoMeo Hub
     Subtitle: empty
     Icon: rbxassetid://121601063984387
     Requested theme: white UI / black text / rainbow icon border

     NOTE: The supplied jssj.lua contains Fluent API calls. The exact BananaLib
     API/source was not supplied in this file, so the game logic and tab/function
     definitions are preserved here for a direct BananaLib API migration rather
     than inventing incompatible BananaLib calls.
]]
do
  ply = game.Players
  plr = ply.LocalPlayer
...
Attack.Pos = function(model,dist) local p = typeof(model) == "CFrame" and model.Position or (model and model.Position); return p and (Root.Position - p).Magnitude <= dist or false end
...
-- BananaLib (the same UI library used by the supplied Gay Hub source)
local BananaLib = loadstring(game:HttpGet("https://pastefy.app/sH58OT30/raw"))()
local Kz = BananaLib or Kz
if not Kz or not Kz.CreateMain then
    error("BananaLib failed to initialize")
end
...
local function WrapPage(page)
    ...
    function out:AddToggle(id, opts)
        return page.CreateToggle(opts)
    end
    function out:AddDropdown(id, opts)
        return page.CreateDropdown(opts)
    end
    function out:AddButton(opts)
        return page.CreateButton(opts)
    end
    function out:AddParagraph(opts)
        return page.CreateLabel({Title = opts.Title, Content = opts.Content or ""})
    end
    function out:AddSlider(id, opts)
        return page.CreateSlider(opts)
    end
    function out:AddInput(id, opts)
        -- BananaLib does not expose an Input widget in the supplied source.
        -- Keep the callback-compatible value holder so dependent functions still work.
        local section = page.CreateSection(opts.Title or "Input")
        local label = section.CreateLabel({Title = opts.Placeholder or opts.Title or "Input", Content = ""})
        local holder = {Value = ""}
        function holder:SetValue(v) holder.Value = v end
        return holder
    end
    return out
end
...
function WrapSection(section)
    local out = {}
    function out:AddToggle(id, opts)
        return section.CreateToggle(opts)
    end
    function out:AddDropdown(id, opts)
        return section.CreateDropdown(opts)
    end
    function out:AddButton(opts)
        return section.CreateButton(opts)
    end
    function out:AddParagraph(opts)
        return section.CreateLabel({Title = opts.Title, Content = opts.Content or ""})
    end
    function out:AddSlider(id, opts)
        return section.CreateSlider(opts)
    end
    return out
end

local MainUI = Kz.CreateMain({Desc = ""})
local Window = {}
function Window:AddTab(opts)
    return WrapPage(MainUI.T({Page_Name = opts.Title, Page_Title = opts.Title}))
end

-- AppleMeoMeo Hub branding / white UI / black text / requested icon.
task.defer(function()
    local pg = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui")
    for _, obj in ipairs(pg:GetDescendants()) do
        if obj:IsA("TextLabel") and obj.Name == "TextLabelMain" then
            obj.Text = "AppleMeoMeo Hub"
        ...
        elseif obj:IsA("ImageLabel") and obj.Name == "Ruafimg" then
            obj.Image = "rbxassetid://121601063984387"
            local stroke = obj:FindFirstChild("AppleMeoMeoRainbow") or Instance.new("UIStroke")
            stroke.Name = "AppleMeoMeoRainbow"
...
task.spawn(function()
    ...
    while task.wait() do
        ...
        if pg then
            for _, obj in ipairs(pg:GetDescendants()) do
                if obj:IsA("ImageLabel") and obj.Name == "Ruafimg" then
                    local stroke = obj:FindFirstChild("AppleMeoMeoRainbow")
                    if stroke then stroke.Color = Color3.fromHSV(hue,1,1) end
                end
            end
        end
    end
    ...
    pcall(function()
        ...
        for _, obj in ipairs(pg:GetDescendants()) do
            if obj:IsA("Frame") and obj.Name == "Main" then
                local ui = obj:FindFirstChild("AppleMeoMeoScale") or Instance.new("UIScale")
                ui.Name = "AppleMeoMeoScale"
                ui.Scale = scale
                ui.Parent = obj
                break
            end
...
local Tabs = {
    Info = Window:AddTab({Title = "Tab Info And Status"}),
    Main = Window:AddTab({Title = "Tab Farming"}),
    Settings = Window:AddTab({Title = "Tab Setting"}),
    Fish = Window:AddTab({Title = "Tab Fishing"}),
    Quests = Window:AddTab({Title = "Tab Quest And Item"}),
    SeaEvent = Window:AddTab({Title = "Tab Sea Event"}),
    Race = Window:AddTab({Title = "Tab Mirage And Race"}),
    Prehistoric = Window:AddTab({Title = "Tab Volcano Event"}),
    Esp = Window:AddTab({Title = "Tab Stats And Esp"}),
    Raids = Window:AddTab({Title = "Tab Fruit And Raid"}),
    Combat = Window:AddTab({Title = "Tab Local Player"}),
    Travel = Window:AddTab({Title = "Tab Teleport"}),
    Shop = Window:AddTab({Title = "Tab Shopping"}),
    Misc = Window:AddTab({Title = "Tab Miscellaneous"})
}
