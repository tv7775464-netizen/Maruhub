...
Attack.Pos = function(model,dist)
  ...
  elseif model then
    ...
    targetPos = hrp and hrp.Position
  end
  return targetPos and Root and (Root.Position - targetPos).Magnitude <= dist or false
end
...
BringEnemy = function(Mon)
    ...
    if not Mon then
        ...
        local all = workspace:FindFirstChild("Enemies")
        if not all then return end
        for _, enemy in ipairs(all:GetChildren()) do
            local hum = enemy:FindFirstChildOfClass("Humanoid")
            local root = enemy:FindFirstChild("HumanoidRootPart")
            if hum and root and hum.Health > 0 then
                local dist = (root.Position - hrp.Position).Magnitude
                if dist < closestDist then
                    closestDist, Mon = dist, enemy
                end
            end
        end
    ...
    pcall(function()
        -- SimulationRadius is expensive when assigned on every attack tick.
        if sethiddenproperty and not _G.__AppleMeoMeoSimRadiusSet then
            sethiddenproperty(plr, "SimulationRadius", math.huge)
            _G.__AppleMeoMeoSimRadiusSet = true
        end

        local targetPos = targetRoot.Position
        local bucket = EnemyCache and EnemyCache[Mon.Name]
        local areaMob = false

        if bucket then
            for v in pairs(bucket) do
                if v ~= Mon and v.Parent == workspace.Enemies then
...
pcall(function()
    if typeof(isnetworkowner) == "function" and root then
        networkOwned = isnetworkowner(root)
    end
end)
        ...
        if targetRoot.CanCollide then targetRoot.CanCollide = false end
        if targetHum.WalkSpeed ~= 0 then targetHum.WalkSpeed = 0 end
        if targetHum.JumpPower ~= 0 then targetHum.JumpPower = 0 end
    end)
end
...
-- Optimized enemy lookup cache. The old version scanned ReplicatedStorage + workspace.Enemies
-- on every call; this hub calls GetConnectionEnemies many times per second.
local function CacheEnemy(v)
  if not v or not v:IsA("Model") then return end
  local hum = v:FindFirstChildOfClass("Humanoid")
  if not hum then return end
  EnemyCache[v.Name] = EnemyCache[v.Name] or {}
  EnemyCache[v.Name][v] = true
end

local function UncacheEnemy(v)
  if not v or not v:IsA("Model") then return end
  local bucket = EnemyCache[v.Name]
  if bucket then
    bucket[v] = nil
    if next(bucket) == nil then EnemyCache[v.Name] = nil end
  end
end

local function WatchEnemyFolder(folder)
  if not folder then return end
  for _,v in ipairs(folder:GetChildren()) do CacheEnemy(v) end
  table.insert(EnemyCacheConnections, folder.ChildAdded:Connect(CacheEnemy))
  table.insert(EnemyCacheConnections, folder.ChildRemoved:Connect(UncacheEnemy))
end

WatchEnemyFolder(workspace:FindFirstChild("Enemies"))
WatchEnemyFolder(replicated)

GetConnectionEnemies = function(a)
  local names = {}
  if typeof(a) == "table" then
    for _,name in ipairs(a) do names[name] = true end
  else
    names[a] = true
  end

  for name in pairs(names) do
    local bucket = EnemyCache[name]
    if bucket then
      for v in pairs(bucket) do
        if v.Parent and Attack.Alive(v) and v:FindFirstChild("HumanoidRootPart") then
          return v
        end
        if not v.Parent then bucket[v] = nil end
      end
    end
  end
end
...
local Fluent, SaveManager, InterfaceManager = loadstring(game:HttpGet("https://raw.githubusercontent.com/tv7775464-netizen/Maruhub/refs/heads/main/GET_modified.lua"))()
...
local Window = Fluent:CreateWindow({
    Title = "apple meomeo Hub",
    SubTitle = "by realthanh",
...
-- Fluent Maru compatibility layer for the original OK Hub controls.
local function wrapTab(tab)
    local w = {}
    function w:AddSection(title)
        return tab:AddSection(title)
    end
    function w:AddParagraph(title, desc)
        local obj = tab:AddParagraph({Title = tostring(title), Content = tostring(desc or "")})
        local originalSetDesc = obj.SetDesc
        function obj:SetDesc(value)
            if originalSetDesc then
                return originalSetDesc(obj, value)
            elseif obj.SetContent then
                return obj:SetContent(value)
            end
        end
        return obj
    end
    function w:AddToggle(opt)
        local id = tostring(opt.Name or opt.Title or ("Toggle_" .. tostring(math.random(1, 999999))))
        return tab:AddToggle(id, {
            Title = opt.Name or opt.Title or id,
            Description = opt.Description or "",
            Default = opt.Default == true,
            Callback = opt.Callback
        })
    end
    function w:AddDropdown(opt)
        local id = tostring(opt.Name or opt.Title or ("Dropdown_" .. tostring(math.random(1, 999999))))
        return tab:AddDropdown(id, {
            Title = opt.Name or opt.Title or id,
            Description = opt.Description or "",
            Values = opt.Options or opt.Values or {},
            Default = opt.Default,
            Multi = opt.Multi or false,
            Callback = opt.Callback
        })
    end
    function w:AddSlider(opt)
        local id = tostring(opt.Name or opt.Title or ("Slider_" .. tostring(math.random(1, 999999))))
        return tab:AddSlider(id, {
            Title = opt.Name or opt.Title or id,
            Description = opt.Description or "",
            Min = opt.Min or 0,
            Max = opt.Max or 100,
            Default = opt.Default or opt.Min or 0,
            Rounding = opt.Rounding or 0,
            Increment = opt.Increment or 1,
            Callback = opt.Callback
        })
    end
    function w:AddButton(opt)
        return tab:AddButton({
            Title = opt.Name or opt.Title or "Button",
            Description = opt.Description or "",
            Callback = opt.Callback
        })
    end
    function w:AddTextBox(opt)
        local id = tostring(opt.Name or opt.Title or ("Input_" .. tostring(math.random(1, 999999))))
        return tab:AddInput(id, {
            Title = opt.Name or opt.Title or id,
            Description = opt.Description or "",
            Default = opt.Default or "",
            Placeholder = opt.Placeholder or "",
            Finished = opt.Finished == true,
            Callback = opt.Callback
        })
    end
    return w
...
local Tabs = {
    ...
    Main = wrapTab(Window:AddTab({ Title = "Tab Farming", Icon = "rbxassetid://7733960981" })),
    Settings = wrapTab(Window:AddTab({ Title = "Tab Setting", Icon = "rbxassetid://7734053495" })),
    Fish = wrapTab(Window:AddTab({ Title = "Tab Fishing", Icon = "rbxassetid://127664059821666" })),
    Quests = wrapTab(Window:AddTab({ Title = "Tab Quest And Item", Icon = "rbxassetid://13075622619" })),
    SeaEvent = wrapTab(Window:AddTab({ Title = "Tab Sea Event", Icon = "waves" })),
    Race = wrapTab(Window:AddTab({ Title = "Tab Mirage And Race", Icon = "rbxassetid://11162889532" })),
    Prehistoric = wrapTab(Window:AddTab({ Title = "Tab Volcano Event", Icon = "tent" })),
    Esp = wrapTab(Window:AddTab({ Title = "Tab Stats And Esp", Icon = "rbxassetid://7040410130" })),
    Raids = wrapTab(Window:AddTab({ Title = "Tab Fruit And Raid", Icon = "rbxassetid://11155986081" })),
    Combat = wrapTab(Window:AddTab({ Title = "Tab Local Player", Icon = "rbxassetid://13075651575" })),
    Travel = wrapTab(Window:AddTab({ Title = "Tab Teleport", Icon = "locate" })),
    Shop = wrapTab(Window:AddTab({ Title = "Tab Shopping", Icon = "rbxassetid://6031265976" })),
    Misc = wrapTab(Window:AddTab({ Title = "Tab Miscellaneous", Icon = "rbxassetid://10709783577" })),
    Config = wrapTab(Window:AddTab({ Title = "Tab Config", Icon = "save" }))
}


-- Config tab: save/load hub settings and customize the interface.
pcall(function()
    if SaveManager then
        if SaveManager.SetLibrary then SaveManager:SetLibrary(Fluent) end
        if SaveManager.IgnoreThemeSettings then SaveManager:IgnoreThemeSettings() end
        if SaveManager.SetIgnoreIndexes then SaveManager:SetIgnoreIndexes({}) end
        if SaveManager.SetFolder then SaveManager:SetFolder("OKHub") end
        if SaveManager.SetSubFolder then SaveManager:SetSubFolder("Configs") end
...
pcall(function()
    if InterfaceManager then
        if InterfaceManager.SetLibrary then InterfaceManager:SetLibrary(Fluent) end
        if InterfaceManager.SetFolder then InterfaceManager:SetFolder("OKHub") end
        if InterfaceManager.BuildInterfaceSection then
            InterfaceManager:BuildInterfaceSection(Tabs.Config)
        end
    end
end)

Tabs.Config:AddSection("Config")
Tabs.Config:AddParagraph("Config Info", "Save, load and manage your OK Hub settings here.")

-- Requested script image / minimize image.


pcall(function()
    if Tabs and Tabs.Settings then
        Tabs.Settings:AddToggle("ResetTP", {
            Title = "Bypass TP",
            Default = true,
            Callback = function(v)
                ResetTP.Enabled = v
...
pcall(function()
    ...
    Tabs.Info:AddButton({
        Title = "Discord",
        Description = "Nhấn để sao chép link Discord",
        Callback = function()
            if setclipboard then
                setclipboard("https://discord.gg/wSHATKxwT")
            end
        end
    })
    Tabs.Info:AddButton({
        Title = "TikTok",
        Description = "Nhấn để sao chép TikTok",
        Callback = function()
            if setclipboard then
                setclipboard("tiktok.com/@iamthanhf337")
            end
        end
    })
end)

local ScriptImage = "rbxassetid://126048951669906"

function Window:SetUIScale(scale)
    local gui = game:GetService("CoreGui"):FindFirstChild("Fluent")
    if gui then
        local uiScale = gui:FindFirstChildOfClass("UIScale", true)
        if uiScale and tonumber(scale) then uiScale.Scale = tonumber(scale) end
    end
end

function Window:Notify(data)
    return Fluent:Notify(data)
end
        ...
        if _G.Level then
            pcall(function()
                ...
                elseif inSub or level < 2600 then
                    ...
                    -- Fix Auto Farm Level for Submerged Island Lv. 2700-2800.
                    -- The final leveling quest is SubmergedQuest3 / Grand Devotee.
                    if level >= 2700 and level <= 2800 and inSub then
                        questData = {
                            [1] = "Grand Devotee",
                            [2] = 2,
                            [3] = "SubmergedQuest3",
                            [4] = 2700,
                            [5] = "Grand Devotee",
                            [6] = CFrame.new(9640.08789, -1992.44507, 9613.65234)
                        }
                    end
                    ...
                    if not foundMob then
                        -- Mob vừa chết/đang respawn: đi tới spawn của mob để không đứng yên.
                        local movedToSpawn = false
                        ...
                        -- Một số server không có EnemySpawns tương ứng; dùng vị trí farm của quest.
                        if not movedToSpawn then
                            local fallbackPos = questData[6]
                            if fallbackPos then
                                _tp(fallbackPos * CFrame.new(0, 20, 0))
                            end
                        end
                    end
                end
...
-- Kill Aura Dragon Storm
Tabs.Settings:AddToggle({
    Name = "Kill Aura Dragon Storm",
    Description = "Kill nearby enemies around the player",
    Default = false,
    Callback = function(Value)
        _G.KillAuraDragonStorm = Value
    end
})

task.spawn(function()
    while task.wait(0.15) do
        if _G.KillAuraDragonStorm then
            pcall(function()
                local char = plr.Character
                ...
                for _, enemy in ipairs(workspace.Enemies:GetChildren()) do
                    local hum = enemy:FindFirstChildOfClass("Humanoid")
                    local root = enemy:FindFirstChild("HumanoidRootPart")
                    if hum and root and hum.Health > 0 and (root.Position - hrp.Position).Magnitude <= 500 then
                        enemy:BreakJoints()
                        hum.Health = 0
...
-- Apple Meomeo Hub — Fluent 13-function build
-- Requested: Random Fruit, Auto Raid, Auto CDK, Auto Soul Guitar,
-- Auto Trial, Auto V3, Auto V2, Auto Cyborg, Auto Ghoul,
-- Auto Farm Lv, Auto Farm Bone, Auto Farm Katakuri, Auto Chest


-- =========================================================
-- APPLE MEO MEO • LIQUID GLASS BLUE THEME
-- Visual-only layer. Keeps Fluent and all existing features.
-- Corner icon : rbxassetid://126048951669906
-- Front logo  : rbxassetid://102474029927884
-- =========================================================
task.spawn(function()
    task.wait(1.5)
    pcall(function()
        ...
        local function glassStroke(parent, name, thickness, transparency)
            if not parent then return end
            ...
            if not s then
                ...
                s.Color = Color3.fromRGB(205, 238, 255)
        ...
        local function glassGradient(parent)
            if not parent then return end
            local g = parent:FindFirstChild("AppleMeoMeoLiquidGradient")
            if not g then
                g = Instance.new("UIGradient")
                g.Name = "AppleMeoMeoLiquidGradient"
                g.Rotation = 90
                g.Color = ColorSequence.new({
                    ColorSequenceKeypoint.new(0.00, Color3.fromRGB(115, 205, 255)),
                    ColorSequenceKeypoint.new(0.45, Color3.fromRGB(45, 125, 220)),
                    ColorSequenceKeypoint.new(1.00, Color3.fromRGB(15, 55, 125))
                })
        ...
        -- Find title text so we can locate the main Fluent window.
        local titleLabel
        for _, obj in ipairs(gui:GetDescendants()) do
            if obj:IsA("TextLabel") then
                local t = string.lower(obj.Text or "")
                if t:find("apple meomeo hub", 1, true) then
                    titleLabel = obj
                    break
                end
            end
        ...
        local mainFrame
        if titleLabel then
            local p = titleLabel.Parent
            for _ = 1, 10 do
                if not p then break end
                if p:IsA("Frame") and p.AbsoluteSize.X >= 300 and p.AbsoluteSize.Y >= 250 then
                    mainFrame = p
                    break
                end
                p = p.Parent
        ...
        if not mainFrame then
            for _, obj in ipairs(gui:GetDescendants()) do
                if obj:IsA("Frame") and obj.AbsoluteSize.X >= 400 and obj.AbsoluteSize.Y >= 300 then
                    mainFrame = obj
                    break
                end
            end
        ...
        if mainFrame then
            mainFrame.BackgroundTransparency = math.min(mainFrame.BackgroundTransparency + 0.06, 0.22)
            corner(mainFrame, 16, "AppleMeoMeoLiquidCorner")
            glassStroke(mainFrame, "AppleMeoMeoLiquidStroke", 1.25, 0.38)
            glassGradient(mainFrame)
        end

        -- Soft glass treatment for the major panels only.
        local panelCount = 0
        for _, obj in ipairs(gui:GetDescendants()) do
            if obj:IsA("Frame") or obj:IsA("ScrollingFrame") then
                if obj ~= mainFrame and obj.AbsoluteSize.X >= 120 and obj.AbsoluteSize.Y >= 45 then
                    obj.BackgroundTransparency = math.min(obj.BackgroundTransparency + 0.04, 0.30)
                    corner(obj, 10, "AppleMeoMeoPanelCorner")
                    glassStroke(obj, "AppleMeoMeoPanelStroke", 0.9, 0.72)
                    panelCount = panelCount + 1
                    if panelCount >= 18 then break end
                end
            end
        end

        -- Top-left blue icon: 126048951669906.
        if titleLabel and titleLabel.Parent then
            local bar = titleLabel.Parent
            local oldIcon = bar:FindFirstChild("AppleMeoMeoCornerIcon")
            if oldIcon then oldIcon:Destroy() end

            local icon = Instance.new("ImageLabel")
            icon.Name = "AppleMeoMeoCornerIcon"
            icon.BackgroundTransparency = 1
            icon.Image = "rbxassetid://126048951669906"
            icon.Size = UDim2.fromOffset(34, 34)
            icon.Position = UDim2.new(0, 9, 0.5, -17)
            icon.ScaleType = Enum.ScaleType.Fit
            icon.ZIndex = titleLabel.ZIndex + 3
            icon.Parent = bar
            corner(icon, 9, "AppleMeoMeoIconCorner")

            pcall(function()
                titleLabel.Position = UDim2.new(
                    titleLabel.Position.X.Scale,
                    math.max(48, titleLabel.Position.X.Offset),
                    titleLabel.Position.Y.Scale,
                    titleLabel.Position.Y.Offset
                )
            end)
        end

        -- White Apple Meo Meo front/logo: 102474029927884.
        if mainFrame then
            local oldLogo = mainFrame:FindFirstChild("AppleMeoMeoFrontLogo")
            if oldLogo then oldLogo:Destroy() end

            local logo = Instance.new("ImageLabel")
            logo.Name = "AppleMeoMeoFrontLogo"
            logo.BackgroundTransparency = 1
            logo.Image = "rbxassetid://102474029927884"
            logo.ImageTransparency = 0.20
            logo.Size = UDim2.fromOffset(175, 72)
            logo.Position = UDim2.new(1, -190, 0, 6)
...
-- =========================================================
-- Apple Meo Meo • Liquid Glass Reference Style
-- Visual-only pass: blue glass, translucent cards, bright borders.
-- Background/icon: 126048951669906
-- Front/logo: 102474029927884
-- =========================================================
task.spawn(function()
    task.wait(2)
    pcall(function()
        ...
        if not gui then
            ...
            gui = pg and pg:FindFirstChild("Fluent")
        end
        if not gui then return end

        local function addCorner(o, r, n)
            if not o then return end
            if not o:FindFirstChild(n) then
                local c=Instance.new("UICorner")
                c.Name=n; c.CornerRadius=UDim.new(0,r); c.Parent=o
            end
        end
        local function addStroke(o, n, tr, th)
            if not o then return end
            if not o:FindFirstChild(n) then
                local st=Instance.new("UIStroke")
                st.Name=n; st.Color=Color3.fromRGB(190,235,255)
                st.Transparency=tr; st.Thickness=th or 1
                st.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
                st.Parent=o
            end
        end
        local function addGradient(o, n)
            if not o or o:FindFirstChild(n) then return end
            local g=Instance.new("UIGradient")
            g.Name=n; g.Rotation=90
            g.Color=ColorSequence.new({
                ColorSequenceKeypoint.new(0,Color3.fromRGB(75,170,245)),
                ColorSequenceKeypoint.new(0.48,Color3.fromRGB(20,95,175)),
                ColorSequenceKeypoint.new(1,Color3.fromRGB(8,48,105))
            })
            g.Transparency=NumberSequence.new({
        ...
        for _,o in ipairs(gui:GetDescendants()) do
            if o:IsA("TextLabel") then
                ...
                if t:find("apple meomeo hub",1,true) then title=o break end
            end
        end

        local root
        if title then
            local q=title.Parent
            for _=1,10 do
                if not q then break end
                if q:IsA("Frame") and q.AbsoluteSize.X>600 and q.AbsoluteSize.Y>400 then
                    root=q; break
                end
                q=q.Parent
            end
        ...
        if not root then
            ...
            for _,o in ipairs(gui:GetDescendants()) do
                if o:IsA("Frame") then
                    ...
                    if a>best then best=a; root=o end
                end
            end
        end
        if not root then return end
        ...
        root.BackgroundColor3=Color3.fromRGB(18,95,165)
        addCorner(root,18,"AMM_GlassCorner")
        addStroke(root,"AMM_GlassStroke",0.22,1.35)
        addGradient(root,"AMM_GlassGradient")

        -- Soft background image inside the window.
        local bg=root:FindFirstChild("AMM_Background")
        if not bg then
            bg=Instance.new("ImageLabel")
            bg.Name="AMM_Background"
            bg.BackgroundTransparency=1
            bg.Image="rbxassetid://126048951669906"
            bg.ImageTransparency=0.58
            bg.ScaleType=Enum.ScaleType.Crop
            bg.Size=UDim2.fromScale(1,1)
            bg.Position=UDim2.fromScale(0,0)
            bg.ZIndex=0
            bg.Parent=root
            addCorner(bg,18,"AMM_BackgroundCorner")
        end

        -- Keep existing UI above the background.
        for _,o in ipairs(root:GetDescendants()) do
            if o:IsA("GuiObject") and o~=bg then
                if o.ZIndex < 2 then o.ZIndex=2 end
            end
        end

        -- Header logo/front.
        if title and title.Parent then
            local bar=title.Parent
            local icon=bar:FindFirstChild("AMM_LogoIcon")
            if not icon then
                icon=Instance.new("ImageLabel")
                icon.Name="AMM_LogoIcon"
                icon.BackgroundTransparency=1
                icon.Image="rbxassetid://126048951669906"
                ...
                icon.ZIndex=20
                icon.Parent=bar
                addCorner(icon,11,"AMM_IconCorner")
            end
            local logo=root:FindFirstChild("AMM_FrontLogo")
            if not logo then
                logo=Instance.new("ImageLabel")
                logo.Name="AMM_FrontLogo"
                logo.BackgroundTransparency=1
                logo.Image="rbxassetid://102474029927884"
                logo.Size=UDim2.fromOffset(250,78)
                logo.Position=UDim2.fromOffset(62,7)
                logo.ScaleType=Enum.ScaleType.Fit
        ...
        -- Card-like translucent panels, matching the reference.
        for _,o in ipairs(root:GetDescendants()) do
            if (o:IsA("Frame") or o:IsA("ScrollingFrame")) and o~=root then
                local w,h=o.AbsoluteSize.X,o.AbsoluteSize.Y
                if w>=250 and h>=70 then
                    o.BackgroundColor3=Color3.fromRGB(28,105,175)
                    o.BackgroundTransparency=math.clamp(o.BackgroundTransparency+0.18,0.18,0.52)
                    addCorner(o,14,"AMM_CardCorner")
                    addStroke(o,"AMM_CardStroke",0.48,1)
                end
            end
        end
    end)
